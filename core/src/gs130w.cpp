 ...
 *
 * GS130W 门面：把 Eeprom + Pipeline + Imu 封装成 C ABI。
 */
#include "gs130w.h"
...
using namespace gs130w;

// IMU 包缓存上限：超过说明 FSYNC 信号丢失
constexpr size_t kMaxImuCache = 1000;

/* 内部 Status → C 错误码 */
static gs130w_err_t to_err(Status st)
{
    switch (st) {
    case Status::Ok:          return GS130W_OK;
    case Status::ParamError:  return GS130W_PARAM_ERROR;
    case Status::Unsupported: return GS130W_UNSUPPORTED;
    case Status::NotFound:    return GS130W_NOT_FOUND;
    case Status::HwError:     return GS130W_HW_ERROR;
    case Status::Timeout:     return GS130W_TIMEOUT;
    }
    return GS130W_HW_ERROR;
}

/* 内部句柄 */
struct gs130w_device_s {
    ...
    std::unique_ptr<base::Fifo<std::array<gs130w_image_nv12_t, 2>>> camera_fifo;   // [CamIndex::Right]=0, [CamIndex::Left]=1
    std::unique_ptr<base::Fifo<gs130w_imu_packet_t>> imu_fifo;
...
static void imu_thread_func(gs130w_device_t *dev)
{
    ...
    std::vector<gs130w_imu_packet_t> pending;   // 弹出时间戳暂存，按旧到新入队
...
static void camera_thread_func(gs130w_device_t *dev)
{
    ...
    while(dev->running.load()) {
        std::array<gs130w_image_nv12_t, 2> f{};
        Status st;
...
{
    return new gs130w_device_t;
}

void gs130w_destroy(gs130w_device_t *dev)
{
    delete dev;
}
...
{
    if(dev == nullptr || cfg == nullptr)return GS130W_PARAM_ERROR;
    if(dev->eeprom || dev->pipeline || dev->imu)return GS130W_PARAM_ERROR;   // 已 init
    ...
    if(!*dev->pipeline)return GS130W_NOT_FOUND;

    // 创建 FIFO（depth < 2 时 Fifo 构造会自动置无效）
    dev->camera_fifo.reset(new base::Fifo<std::array<gs130w_image_nv12_t, 2>>(
        cfg->camera_fifo.depth, static_cast<FifoMode>(cfg->camera_fifo.mode)));
    dev->imu_fifo.reset(new base::Fifo<gs130w_imu_packet_t>(
    ...
    dev->fsync_camera = (cfg->camera_config.fsync_camera == GS130W_CAMERA_RIGHT_IDX)
    ...
    // 初始化 Camera（建流配置，不开流）
    {
        const gs130w_camera_config_t &cc = cfg->camera_config;
        PipelineConfig pc{};
        ...
        if(cal == nullptr && cc.mode != GS130W_CAMERA_MODE_RAW)return GS130W_PARAM_ERROR;
...
gs130w_err_t gs130w_start(gs130w_device_t *dev)
{
    if(dev == nullptr)return GS130W_PARAM_ERROR;
    if(dev->started)return GS130W_PARAM_ERROR;
    if(!dev->pipeline)return GS130W_PARAM_ERROR;
...
void gs130w_stop(gs130w_device_t *dev)
{
...
gs130w_err_t gs130w_deinit(gs130w_device_t *dev)
{
    if(dev == nullptr)return GS130W_PARAM_ERROR;

    if(dev->started)gs130w_stop(dev);
...
size_t gs130w_available_camera(gs130w_device_t *dev)
{
    if(dev == nullptr || !dev->camera_fifo)return 0;
    return dev->camera_fifo->size();
}

gs130w_err_t gs130w_get_nv12_frame(gs130w_device_t *dev,
                                   gs130w_image_nv12_t *image_left,
                                   gs130w_image_nv12_t *image_right)
{
    if(dev == nullptr || image_left == nullptr || image_right == nullptr)
        return GS130W_PARAM_ERROR;
    if(!dev->camera_fifo)return GS130W_PARAM_ERROR;

    std::array<gs130w_image_nv12_t, 2> f;
    if(!dev->camera_fifo->pop(f))return GS130W_TIMEOUT;
...
size_t gs130w_available_imu(gs130w_device_t *dev)
{
...
gs130w_err_t gs130w_read_imu(gs130w_device_t *dev,
                             gs130w_imu_packet_t *out)
{
    if(dev == nullptr || out == nullptr)return GS130W_PARAM_ERROR;
    if(!dev->imu || !dev->imu_fifo)return GS130W_PARAM_ERROR;
    if(dev->imu_fault.load())return GS130W_HW_ERROR;
    if(!dev->imu_fifo->pop(*out))return GS130W_TIMEOUT;
    return GS130W_OK;
}

/* ---- 标定 ---- */

gs130w_err_t gs130w_get_calibration(gs130w_device_t *dev,
                                    gs130w_calibration_t *calibration)
{
    if(dev == nullptr || calibration == nullptr)return GS130W_PARAM_ERROR;
    if(!dev->eeprom)return GS130W_NOT_FOUND;
